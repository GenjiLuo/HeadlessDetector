...
async function testBrowser(name, testFunction) {
  ...
  const passedBlock = document.getElementById(`${name}-passed-result`);

  result = await testFunction(resultBlock);
  if (result == HEADLESS){
    //row.classList.add("headless");
    passedBlock.classList.add("headless");
    passedBlock.innerHTML="headless";
  }
  else if (result == HEADFUL){
    //row.classList.add("headful");
    passedBlock.classList.add("headful");
    passedBlock.innerHTML="headful";
  }
  else{
    //row.classList.add("undefined");
    passedBlock.classList.add("undefined");
    passedBlock.innerHTML="not sure";
  }
}
 ...
 */
const tests = [
  { name: "User Agent<sup>Old & Important<sup>",        id: "user-agent",        testFunction: testUserAgent        },
  { name: "App Version<sup>Old<sup>",                   id: "app-version",       testFunction: testAppVersion       },
  { name: "Plugins<sup>Old & Important<sup>",           id: "plugins",           testFunction: testPlugins          },
  { name: "Plugins Prototype<sup>Old & Important<sup>", id: "plugins-prototype", testFunction: testPluginsPrototype },
  { name: "Mime<sup>Old & Important<sup>",              id: "mime",              testFunction: testMime             },
  { name: "Mime Prototype<sup>Old & Important<sup>",    id: "mime-prototype",    testFunction: testMimePrototype    },
  { name: "Languages<sup>New<sup>",                     id: "languages",         testFunction: testLanguages        },
  { name: "Webdriver<sup>new & Important<sup>",         id: "webdriver",         testFunction: testWebdriver        },
 // { name: "<sup>Old<sup>Time Elapse",       id: "time-elapse",       testFunction: testTimeElapse       },
  { name: "Chrome<sup>New & Important<sup>",            id: "chrome-element",    testFunction: testChrome           },
  { name: "Permission<sup>New & Important<sup>",        id: "permission",        testFunction: testPermission       },
  { name: "Devtool Protocol<sup>New & Important<sup>",  id: "devtool",           testFunction: testDevtool          },
  { name: "Broken Image<sup>Old & Important<sup>",      id: "image",             testFunction: testImage            },
  { name: "Outer dimensions<sup>Old & Important<sup>",  id: "outer",             testFunction: testOuter            },
  { name: "Connection Rtt<sup>Old & Important<sup>",    id: "connection-rtt",    testFunction: testConnectionRtt     },
  { name: "Mouse Move<sup>Old<sup>",                    id: "mouse-move",        testFunction: testMouseMove        },
];
...
//  let row = table.insertRow();
  ...
  nameBlock.style = "color:#666;";
  let resultBlock = row.insertCell();
  let passedBlock = row.insertCell();
  writeToBlock(nameBlock, name);
  resultBlock.id = `${id}-result`;
  passedBlock.id = `${id}-passed-result`;
}
